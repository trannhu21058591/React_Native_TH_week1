var marksBMI, johnBMI;

function markHigherBMI(mass_marks, height_marks, mass_john, height_john) {
    marksBMI = mass_marks / height_marks ** 2;
    johnBMI = mass_john / height_john ** 2;
    return marksBMI > johnBMI;
}

var markHigherBMI_data1 = markHigherBMI(78, 1.69, 92, 1.95); // truyền dữ liệu vào hàm
console.log("Mark co chi so lon hon John: ", markHigherBMI_data1);

// Use the BMI example from Challenge #1, and the code you already wrote, and
// improve it.
// Your tasks:
// 1. Print a nice output to the console, saying who has the higher BMI. The message
// is either "Mark's BMI is higher than John's!" or "John's BMI is higher than Mark's!"
// 2. Use a template literal to include the BMI values in the outputs. Example: "Mark's
// BMI (28.3) is higher than John's (23.9)!"
// Hint: Use an if/else statement

if (markHigherBMI_data1) {
    console.log(`Mark' s BMI (${marksBMI.toFixed(2)}) is higher than John's BMI (${johnBMI.toFixed(2)})`);
} else console.log(`Mark' s BMI (${marksBMI.toFixed(2)}) is lower than than John's BMI (${johnBMI.toFixed(2)})`);